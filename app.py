...
st.set_page_config(
    page_title="Doctor's Precription Reader",
    page_icon="🔍",
    layout="wide",
...
def process_image(image, filename, fields=None, model=None):
    ...
    if fields is None:
        ...
        prompt = "Extract only the medicine names, dosages, and frequency from the following text"
...
with st.sidebar:
    ...
    extraction_mode = "Generate medical Prescription"
    ...
    if uploaded_files:
        ...
        st.header("Medical Extraction ")
        extraction_mode = st.radio(
            "",
            ["Extract medical info"]
        )
    
        process_button = st.button("Extract")
    else:
